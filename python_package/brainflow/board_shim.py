...
class BoardShim(object):
    ...
    @classmethod
    def get_sampling_rate(cls, board_id: int, preset: str = 'default') -> int:
        ...
        try:
            preset_str = preset.encode()
        except BaseException:
            preset_str = preset
        sampling_rate = numpy.zeros(1).astype(numpy.int32)
        res = BoardControllerDLL.get_instance().get_sampling_rate(board_id, preset_str, sampling_rate)
        if res != BrainFlowExitCodes.STATUS_OK.value:
            raise BrainFlowError('unable to request info about this board', res)
        return int(sampling_rate[0])

    @classmethod
    def get_package_num_channel(cls, board_id: int, preset: str = 'default') -> int:
        ...
        res = BoardControllerDLL.get_instance().get_package_num_channel(board_id, preset_str, package_num_channel)
        if res != BrainFlowExitCodes.STATUS_OK.value:
            raise BrainFlowError('unable to request info about this board', res)
        return int(package_num_channel[0])

    @classmethod
    def get_battery_channel(cls, board_id: int, preset: str = 'default') -> int:
        ...
        res = BoardControllerDLL.get_instance().get_battery_channel(board_id, preset_str, battery_channel)
        if res != BrainFlowExitCodes.STATUS_OK.value:
            raise BrainFlowError('unable to request info about this board', res)
        return int(battery_channel[0])

    @classmethod
    def get_num_rows(cls, board_id: int, preset: str = 'default') -> int:
        ...
        res = BoardControllerDLL.get_instance().get_num_rows(board_id, preset_str, num_rows)
        if res != BrainFlowExitCodes.STATUS_OK.value:
            raise BrainFlowError('unable to request info about this board', res)
        return int(num_rows[0])

    @classmethod
    def get_timestamp_channel(cls, board_id: int, preset: str = 'default') -> int:
        ...
        res = BoardControllerDLL.get_instance().get_timestamp_channel(board_id, preset_str, timestamp_channel)
        if res != BrainFlowExitCodes.STATUS_OK.value:
            raise BrainFlowError('unable to request info about this board', res)
        return int(timestamp_channel[0])

    @classmethod
    def get_marker_channel(cls, board_id: int, preset: str = 'default') -> int:
        ...
        res = BoardControllerDLL.get_instance().get_marker_channel(board_id, preset_str, marker_channel)
        if res != BrainFlowExitCodes.STATUS_OK.value:
            raise BrainFlowError('unable to request info about this board', res)
        return int(marker_channel[0])

    @classmethod
    def get_eeg_names(cls, board_id: int, preset: str = 'default') -> List[str]:
        ...
        res = BoardControllerDLL.get_instance().get_eeg_names(board_id, preset_str, string, string_len)
        if res != BrainFlowExitCodes.STATUS_OK.value:
            raise BrainFlowError('unable to request info about this board', res)
    ...
    def get_board_presets(cls, board_id: int) -> List[str]:
        ...
        string = numpy.zeros(4096).astype(numpy.ubyte)
        string_len = numpy.zeros(1).astype(numpy.int32)
        res = BoardControllerDLL.get_instance().get_board_presets(board_id, 4096, string, string_len)
        if res != BrainFlowExitCodes.STATUS_OK.value:
            raise BrainFlowError('unable to request info about this board', res)
        return string.tobytes().decode('utf-8')[0:string_len[0]].split(',')
    ...
    @classmethod
    def get_board_descr(cls, board_id: int, preset: str = 'default'):
        ...
        string_len = numpy.zeros(1).astype(numpy.int32)
        res = BoardControllerDLL.get_instance().get_board_descr(board_id, preset_str, string, string_len)
        if res != BrainFlowExitCodes.STATUS_OK.value:
            raise BrainFlowError('unable to request info about this board', res)
        return json.loads(string.tobytes().decode('utf-8')[0:string_len[0]])

    @classmethod
    def get_device_name(cls, board_id: int, preset: str = 'default') -> str:
        """get device name
        ...
        string = numpy.zeros(4096).astype(numpy.ubyte)
        string_len = numpy.zeros(1).astype(numpy.int32)
        res = BoardControllerDLL.get_instance().get_device_name(board_id, preset_str, string, string_len)
        if res != BrainFlowExitCodes.STATUS_OK.value:
            raise BrainFlowError('unable to request info about this board', res)
        return string.tobytes().decode('utf-8')[0:string_len[0]]

    @classmethod
    def get_eeg_channels(cls, board_id: int, preset: str = 'default') -> List[int]:
        ...
        res = BoardControllerDLL.get_instance().get_eeg_channels(board_id, preset_str, eeg_channels, num_channels)
        if res != BrainFlowExitCodes.STATUS_OK.value:
            raise BrainFlowError('unable to request info about this board', res)
    ...
    @classmethod
    def get_exg_channels(cls, board_id: int, preset: str = 'default') -> List[int]:
        ...
        res = BoardControllerDLL.get_instance().get_exg_channels(board_id, preset_str, exg_channels, num_channels)
        if res != BrainFlowExitCodes.STATUS_OK.value:
            raise BrainFlowError('unable to request info about this board', res)
    ...
    @classmethod
    def get_emg_channels(cls, board_id: int, preset: str = 'default') -> List[int]:
        ...
        res = BoardControllerDLL.get_instance().get_emg_channels(board_id, preset_str, emg_channels, num_channels)
        if res != BrainFlowExitCodes.STATUS_OK.value:
            raise BrainFlowError('unable to request info about this board', res)
    ...
    @classmethod
    def get_ecg_channels(cls, board_id: int, preset: str = 'default') -> List[int]:
        ...
        res = BoardControllerDLL.get_instance().get_ecg_channels(board_id, preset_str, ecg_channels, num_channels)
        if res != BrainFlowExitCodes.STATUS_OK.value:
            raise BrainFlowError('unable to request info about this board', res)
    ...
    @classmethod
    def get_eog_channels(cls, board_id: int, preset: str = 'default') -> List[int]:
        ...
        res = BoardControllerDLL.get_instance().get_eog_channels(board_id, preset_str, eog_channels, num_channels)
        if res != BrainFlowExitCodes.STATUS_OK.value:
            raise BrainFlowError('unable to request info about this board', res)
    ...
    @classmethod
    def get_eda_channels(cls, board_id: int, preset: str = 'default') -> List[int]:
        ...
        res = BoardControllerDLL.get_instance().get_eda_channels(board_id, preset_str, eda_channels, num_channels)
        if res != BrainFlowExitCodes.STATUS_OK.value:
            raise BrainFlowError('unable to request info about this board', res)
    ...
    @classmethod
    def get_ppg_channels(cls, board_id: int, preset: str = 'default') -> List[int]:
        ...
        res = BoardControllerDLL.get_instance().get_ppg_channels(board_id, preset_str, ppg_channels, num_channels)
        if res != BrainFlowExitCodes.STATUS_OK.value:
            raise BrainFlowError('unable to request info about this board', res)
    ...
    @classmethod
    def get_accel_channels(cls, board_id: int, preset: str = 'default') -> List[int]:
        ...
        res = BoardControllerDLL.get_instance().get_accel_channels(board_id, preset_str, accel_channels, num_channels)
        if res != BrainFlowExitCodes.STATUS_OK.value:
            raise BrainFlowError('unable to request info about this board', res)
    ...
    @classmethod
    def get_analog_channels(cls, board_id: int, preset: str = 'default') -> List[int]:
        ...
        res = BoardControllerDLL.get_instance().get_analog_channels(board_id, preset_str, analog_channels, num_channels)
        if res != BrainFlowExitCodes.STATUS_OK.value:
            raise BrainFlowError('unable to request info about this board', res)
    ...
    @classmethod
    def get_gyro_channels(cls, board_id: int, preset: str = 'default') -> List[int]:
        ...
        res = BoardControllerDLL.get_instance().get_gyro_channels(board_id, preset_str, gyro_channels, num_channels)
        if res != BrainFlowExitCodes.STATUS_OK.value:
            raise BrainFlowError('unable to request info about this board', res)
    ...
    @classmethod
    def get_other_channels(cls, board_id: int, preset: str = 'default') -> List[int]:
        ...
        res = BoardControllerDLL.get_instance().get_other_channels(board_id, preset_str, other_channels, num_channels)
        if res != BrainFlowExitCodes.STATUS_OK.value:
            raise BrainFlowError('unable to request info about this board', res)
    ...
    @classmethod
    def get_temperature_channels(cls, board_id: int, preset: str = 'default') -> List[int]:
        ...
        res = BoardControllerDLL.get_instance().get_temperature_channels(board_id, preset_str, temperature_channels, num_channels)
        if res != BrainFlowExitCodes.STATUS_OK.value:
            raise BrainFlowError('unable to request info about this board', res)
    ...
    @classmethod
    def get_resistance_channels(cls, board_id: int, preset: str = 'default') -> List[int]:
        ...
        res = BoardControllerDLL.get_instance().get_resistance_channels(board_id, preset_str, resistance_channels, num_channels)
        if res != BrainFlowExitCodes.STATUS_OK.value:
            raise BrainFlowError('unable to request info about this board', res)
    ...
    def get_current_board_data(self, num_samples: int, preset: str = 'default') -> NDArray[Float64]:
        ...
        res = BoardControllerDLL.get_instance().get_current_board_data(num_samples, preset_str, data_arr, current_size,
    ...
    def get_board_data_count(self, preset: str = 'default') -> int:
        ...
        res = BoardControllerDLL.get_instance().get_board_data_count(preset_str, data_size, self.board_id, self.input_json)
        if res != BrainFlowExitCodes.STATUS_OK.value:
    ...
    def insert_marker(self, value: float, preset: str = 'default') -> None:
        ...
        try:
            ...
            preset_str = preset
        res = BoardControllerDLL.get_instance().insert_marker(value, preset_str, self.board_id, self.input_json)
        if res != BrainFlowExitCodes.STATUS_OK.value:
    ...
    def get_board_data(self, num_samples=None, preset: str = 'default') -> NDArray[Float64]:
        ...
        res = BoardControllerDLL.get_instance().get_board_data(data_size, preset_str, data_arr, self.board_id, self.input_json)
        if res != BrainFlowExitCodes.STATUS_OK.value:
