...
class EEGDataVisualizer(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        # 创建连接设备按钮
        self.connect_button = QtWidgets.QPushButton('Connect the electroencephalogram (EEG) device')
        self.connect_button.clicked.connect(self.connect_device)
        left_layout.addWidget(self.connect_button)

        # 创建开始采集按钮（初始不可用，连接设备后可用）
        self.start_button = QtWidgets.QPushButton('Start collecting data')
        ...
        self.setWindowTitle('Electroencephalogram (EEG) Data Demonstration Interface')
    ...
    def start_real_time_collection(self):
        try:
            ...
            if len(self.eeg_channels) > 0:
                ...
                raise ValueError("An exception occurred while obtaining the number of electroencephalogram (EEG) channels, and the data buffer cannot be initialized")
    ...
    def timerEvent(self, event):
        if not self.paused and not self.stop and not self.timer_stopped:  # 增加判断定时器是否停止的条件
            # 获取一定数量的新数据点
    ...
    def stop_real_time_collection(self):
        ...
        if self.board_shim is not None:
            try:
                ...
                #     self.board_shim.release_session()
                #     print("The data stream has been successfully stopped and the session resources have been released")
            except Exception as e:
                print(f"An error occurred while stopping the data stream or releasing the session resources: {e}")
    ...
    def update_graph(self):
        ...
        # 先统一收集所有要绘制通道的滤波后数据，方便后续统一处理坐标轴范围
        all_channel_data = []
        for channel in range(len(self.eeg_channels)):
            if self.channel_checkboxes[channel].isChecked():
                valid_length = min(len(self.data_buffer[0]), self.buffer_index)
                channel_data = self.data_buffer[channel, :valid_length]
                all_channel_data.append(channel_data)

        # 根据所选滤波器类型，调整坐标轴范围（幅度 - y轴）
        if any([checkbox_info["checkbox"].isChecked() for _, checkbox_info in self.filter_checkboxes.items()]):
            y_data = np.concatenate(all_channel_data) if all_channel_data else np.array([])
            y_min, y_max = np.min(y_data), np.max(y_data)
            y_margin = 0.1 * (y_max - y_min)
            self.ax.set_ylim([y_min - y_margin, y_max + y_margin])

        # 计算时间轴，考虑滤波后数据长度变化情况（如果有），这里假设数据长度没有变化，可根据实际调整
        time_axis = np.arange(0, self.buffer_index) / sampling_rate
    ...
    def apply_filter_to_data(self, sampling_rate, filter_type):
        try:
            ...
            print(f"input {filter_type} the cut-off frequency parameter of the filter is invalid. Please enter a valid number！")
