...
void Proxy::interfaces_unload(SimpleDBusLegacy::Holder removed_interfaces) {
    std::scoped_lock lock(_interface_access_mutex);
...
void Proxy::path_add(const std::string& path, SimpleDBusLegacy::Holder managed_interfaces) {
...
bool Proxy::path_remove(const std::string& path, SimpleDBusLegacy::Holder options) {
...
Holder Proxy::path_collect() {
    SimpleDBusLegacy::Holder result = SimpleDBusLegacy::Holder::create_dict();
    SimpleDBusLegacy::Holder interfaces = SimpleDBusLegacy::Holder::create_dict();

    for (const auto& [interface_name, interface_ptr] : _interfaces) {
        SimpleDBusLegacy::Holder properties = interface_ptr->property_collect();
        interfaces.dict_append(SimpleDBusLegacy::Holder::Type::STRING, interface_name, std::move(properties));
    }

    if (!interfaces.get_dict_string().empty()) {
        result.dict_append(SimpleDBusLegacy::Holder::Type::OBJ_PATH, _path, std::move(interfaces));
    }

    for (const auto& [child_path, child] : _children) {
        SimpleDBusLegacy::Holder child_result = child->path_collect();
        // Merge child_result into result
        for (auto&& [path, child_interfaces] : child_result.get_dict_object_path()) {
            result.dict_append(SimpleDBusLegacy::Holder::Type::OBJ_PATH, std::move(path), std::move(child_interfaces));
        }
    }
