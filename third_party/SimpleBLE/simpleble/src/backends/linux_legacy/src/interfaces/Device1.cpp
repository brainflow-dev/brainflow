...
const SimpleDBusLegacy::AutoRegisterInterface<Device1> Device1::registry{
    "org.bluez.Device1",
    // clang-format off
    [](std::shared_ptr<SimpleDBusLegacy::Connection> conn, const std::string& bus_name, const std::string& path, const SimpleDBusLegacy::Holder& options) -> std::shared_ptr<SimpleDBusLegacy::Interface> {
        return std::static_pointer_cast<SimpleDBusLegacy::Interface>(std::make_shared<Device1>(conn, path));
    }
    // clang-format on
};

Device1::Device1(std::shared_ptr<SimpleDBusLegacy::Connection> conn, std::string path)
    : SimpleDBusLegacy::Interface(conn, "org.bluez", path, "org.bluez.Device1") {}
...
std::vector<std::string> Device1::UUIDs() {
    ...
    for (SimpleDBusLegacy::Holder& uuid : _properties["UUIDs"].get_array()) {
...
void Device1::property_changed(std::string option_name) {
    if (option_name == "Connected") {
        ...
        std::map<uint16_t, SimpleDBusLegacy::Holder> manuf_data = _properties["ManufacturerData"].get_dict_uint16();
        ...
        _service_data.clear();
        std::map<std::string, SimpleDBusLegacy::Holder> service_data = _properties["ServiceData"].get_dict_string();
        // Loop through all received keys and store them.
