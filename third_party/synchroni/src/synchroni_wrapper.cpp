...
class SynchroniBoardWrapper: public sensor::SensorProfileDelegate, public enable_shared_from_this<SynchroniBoardWrapper>{
public:
    ...
    virtual void onErrorCallback(std::shared_ptr<sensor::SensorProfile> profile, std::string errorMsg) {
        
    }
    
    virtual void onStateChange(std::shared_ptr<sensor::SensorProfile> profile, sensor::BLEDevice::State newState){
        
        //throw BrainFlowException ("failed to get board info", res);
    }
    
    virtual void onSensorNotifyData(std::shared_ptr<sensor::SensorProfile> profile, const sensor::SensorData& rawData){
...
class SynchroniControllerDelegate: public sensor::SensorControllerDelegate, public enable_shared_from_this<SynchroniControllerDelegate>{
public:
...
int synchroni_initialize (void *param){
    std::call_once(initFlag, [](){
        ...
        SynchroniControllerDelegate::sensorControllerDelegate = make_shared<SynchroniControllerDelegate>();
        SensorController::getInstance()->setDelegate(SynchroniControllerDelegate::sensorControllerDelegate);
    });
...
int synchroni_open_device (void *param){
    ...
    if (wrapper->profile == NULL){
        SynchroniControllerDelegate::sensorControllerDelegate->logger->log(spdlog::level::info, "Use timeout for discovery: {}", timeout);
    ...
    if (!wrapper->profile->hasInit()){
        wrapper->profile->init(PACKAGE_COUNT, timeout * 1000, [](bool succeed, string err){
            if (err != ""){
                SynchroniControllerDelegate::sensorControllerDelegate->logger->log(spdlog::level::err, "init error: " + err);
            }
        });
    ...
    wrapper->profile->fetchDeviceInfo(timeout * 1000, [&deviceMatch, eegChannel, &wrapper](DeviceInfo info, string err){
    ...
    SynchroniControllerDelegate::sensorControllerDelegate->logger->log(spdlog::level::info, "Session is prepared");
    return 0;
}
...
int synchroni_start_stream (void *param){
    ...
    wrapper->profile->startDataNotification(timeout * 1000, [&finished, &hasSucceed, &wrapper](bool succeed, string err){
        hasSucceed = succeed;
        if (err != ""){
            SynchroniControllerDelegate::sensorControllerDelegate->logger->log(spdlog::level::err, "Start stream error: " + err);
        }
...
int synchroni_stop_stream (void *param){
    ...
    wrapper->profile->stopDataNotification(timeout * 1000, [&finished, &hasSucceed](bool succeed, string err){
        hasSucceed = succeed;
        if (err != ""){
            SynchroniControllerDelegate::sensorControllerDelegate->logger->log(spdlog::level::err, "Stop stream error: " + err);
        }
        finished = true;
    });
...
int synchroni_config_device (void *param){
    ...
    wrapper->profile->setParam(timeout * 1000, key_char, "", [&finished, result_char, buffer_size](string result, string err){
