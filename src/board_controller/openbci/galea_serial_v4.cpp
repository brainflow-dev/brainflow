...
GaleaSerialV4::GaleaSerialV4 (struct BrainFlowInputParams params)
    : Board ((int)BoardIds::GALEA_SERIAL_BOARD_V4, params)
{
    serial = NULL;
    is_streaming = false;
    keep_alive = false;
...
GaleaSerialV4::~GaleaSerialV4 ()
{
    skip_logs = true;
    release_session ();
}

int GaleaSerialV4::prepare_session ()
{
    // check params
    if (initialized)
    {
...
int GaleaSerialV4::config_board (std::string conf, std::string &response)
{
    if (serial == NULL)
    {
    ...
    safe_logger (spdlog::level::debug, "Trying to config GaleaSerialV4 with {}", new_conf.c_str ());
...
int GaleaSerialV4::start_stream (int buffer_size, const char *streamer_params)
{
    if (!initialized)
    {
        safe_logger (spdlog::level::err, "You need to call prepare_session before config_board");
...
int GaleaSerialV4::release_session ()
{
    if (initialized)
    {
        if (is_streaming)
...
void GaleaSerialV4::read_thread ()
{
    int res = 0;
...
int GaleaSerialV4::calc_time (std::string &resp)
{
