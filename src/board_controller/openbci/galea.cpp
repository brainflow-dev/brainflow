...
int Galea::config_board(std::string conf, std::string &response) {
  if (socket == NULL) {
    safe_logger(spdlog::level::err,
                "You need to call prepare_session before config_board");
  ...
  if (conf == "calc_time") {
    if (is_streaming) {
      safe_logger(spdlog::level::err,
                  "can not calc delay during the streaming.");
      return (int)BrainFlowExitCodes::BOARD_NOT_CREATED_ERROR;
    }
  ...
  if (len != res) {
    if (res == -1) {
#ifdef _WIN32
      safe_logger(spdlog::level::err, "WSAGetLastError is {}",
                  WSAGetLastError());
#else
      safe_logger(spdlog::level::err, "errno {} message {}", errno,
                  strerror(errno));
#endif
    }
  ...
  if (!is_streaming) {
    ...
    while (res == Galea::transaction_size) {
...
#ifdef _WIN32
        safe_logger(spdlog::level::err,
                    "config_board recv ack WSAGetLastError is {}",
                    WSAGetLastError());
#else
        safe_logger(spdlog::level::err,
                    "config_board recv ack errno {} message {}", errno,
                    strerror(errno));
#endif
        return (int)BrainFlowExitCodes::BOARD_WRITE_ERROR;
      }
      ...
      if (current_attempt == max_attempt) {
        safe_logger(spdlog::level::err,
                    "Device is streaming data while it should not!");
...
int Galea::start_stream(int buffer_size, const char *streamer_params) {
  ...
  if (cv.wait_for(lk, 3 * sec, [this] {
    ...
    safe_logger(spdlog::level::err,
                "no data received in 5sec, stopping thread");
    this->is_streaming = true;
    this->stop_stream();
...
int Galea::stop_stream() {
  if (is_streaming) {
    ...
    while (res != -1) {
      ...
      if (current_attempt == max_attempt) {
        safe_logger(spdlog::level::err,
                    "Command 's' was sent but streaming is still running.");
        return (int)BrainFlowExitCodes::BOARD_WRITE_ERROR;
      }
    }
...
void Galea::read_thread() {
  ...
  constexpr int offset_last_package =
      Galea::package_size * (Galea::num_packages - 1);
  ...
  while (keep_alive) {
    ...
    for (int cur_package = 0; cur_package < Galea::num_packages;
         cur_package++) {
      ...
      for (int i = 4, tmp_counter = 0; i < 20; i++, tmp_counter++) {
        ...
        if (tmp_counter < 6)
          package[i - 3] = emg_scale * (double)cast_24bit_to_int32(
                                           b + offset + 5 + 3 * (i - 4));
      ...
      // temperature
      package[board_descr["temperature_channels"][0].get<int>()] =
          temperature / 100.0;
      // battery
...
int Galea::calc_time(std::string &resp) {
  ...
  if (res != bytes_to_calc_rtt) {
    safe_logger(spdlog::level::warn,
                "failed to send time calc command to device");
    return (int)BrainFlowExitCodes::BOARD_WRITE_ERROR;
  }
  ...
  if (res != bytes_to_calc_rtt) {
    safe_logger(spdlog::level::warn,
                "failed to recv resp from time calc command, resp size {}",
                res);
    return (int)BrainFlowExitCodes::BOARD_WRITE_ERROR;
  }
    ...
    if (res == msearch.size()) {
      ...
      if (res > 1) {
        ...
        if (std::regex_search(response, matches, rgx) == true) {
          ...
          safe_logger(spdlog::level::err,
                      "failed to find ip address in response");
        }
      } else {
