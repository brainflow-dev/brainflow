...
AntNeuroEdxInternalBoard::AntNeuroEdxInternalBoard (
...
int AntNeuroEdxInternalBoard::create_grpc_stub ()
{
...
int AntNeuroEdxInternalBoard::discover_and_create_amplifier ()
{
...
int AntNeuroEdxInternalBoard::load_capabilities ()
{
    ...
    {
        sampling_rate =
            *std::max_element (sampling_rates_available.begin (), sampling_rates_available.end ());
    }
...
int AntNeuroEdxInternalBoard::prepare_session ()
{
    if (initialized)
    {
        return (int)BrainFlowExitCodes::STATUS_OK;
    ...
    if (!sampling_rates_available.empty () &&
        std::find (sampling_rates_available.begin (), sampling_rates_available.end (),
            expected_rate) == sampling_rates_available.end ())
    {
        ...
        safe_logger (spdlog::level::err,
            ...
            expected_rate, rates_stream.str ());
        release_session ();
        return (int)BrainFlowExitCodes::INVALID_ARGUMENTS_ERROR;
    }
...
int AntNeuroEdxInternalBoard::configure_stream_params (void *request_ptr)
{
...
int AntNeuroEdxInternalBoard::set_mode ()
{
    EdigRPC::gen::Amplifier_SetModeRequest request;
...
int AntNeuroEdxInternalBoard::set_idle_mode ()
{
...
int AntNeuroEdxInternalBoard::apply_mode_change ()
{
    if (!initialized)
    {
        return (int)BrainFlowExitCodes::BOARD_NOT_READY_ERROR;
    ...
    if (was_streaming)
    {
        safe_logger (spdlog::level::info,
            "EDX apply_mode_change: stopping current stream (impedance_mode={})",
            impedance_mode ? 1 : 0);
        keep_alive = false;
        is_streaming = false;
    ...
    safe_logger (
        spdlog::level::info, "EDX apply_mode_change: waiting up to {}s for first frame", wait_secs);
    std::unique_lock<std::mutex> lk (wait_mutex);
    if (wait_cv.wait_for (lk, std::chrono::seconds (wait_secs),
    ...
    safe_logger (spdlog::level::err,
        "EDX apply_mode_change: timed out after {}s waiting for first frame", wait_secs);
    keep_alive = false;
    if (streaming_thread.joinable ())
    {
        streaming_thread.join ();
...
void AntNeuroEdxInternalBoard::read_thread ()
{
...
void AntNeuroEdxInternalBoard::process_mode_request ()
{
...
int AntNeuroEdxInternalBoard::process_frames ()
{
    ...
    std::vector<int> ref_resistance_channels = try_get_vec (frame_descr, "ref_resistance_channels");
    std::vector<int> gnd_resistance_channels = try_get_vec (frame_descr, "gnd_resistance_channels");
    ...
    for (const auto &frame : response.framelist ())
    {
        ...
        if (impedance_frame)
        {
            ...
            for (size_t i = 0;
                i < frame.impedance ().channels ().size () && i < resistance_channels.size (); i++)
            {
                package[(size_t)resistance_channels[i]] =
                    (double)frame.impedance ().channels ((int)i).value ();
            }
            for (size_t i = 0;
                i < frame.impedance ().reference ().size () && i < ref_resistance_channels.size ();
                i++)
            {
            ...
            for (size_t i = 0;
                i < frame.impedance ().ground ().size () && i < gnd_resistance_channels.size ();
                i++)
            {
        ...
        if (frame_marker_count > 0 && has_start && sample_dt > 0.0)
        {
            for (int mi = 0; mi < frame_marker_count; mi++)
            {
                ...
                int target_row =
                    (int)std::max (0LL, std::min ((long long)(rows - 1), target_row_ll));
...
int AntNeuroEdxInternalBoard::start_stream (int buffer_size, const char *streamer_params)
{
    if (!initialized)
    {
        return (int)BrainFlowExitCodes::BOARD_NOT_READY_ERROR;
...
int AntNeuroEdxInternalBoard::request_mode_from_thread (EdxModeRequest req)
{
    if (!keep_alive)
    {
    ...
    if (mode_cv.wait_for (
            lk, deadline, [this] { return mode_request.load () == EdxModeRequest::None; }))
    {
...
int AntNeuroEdxInternalBoard::stop_stream ()
{
...
int AntNeuroEdxInternalBoard::release_session ()
{
    if (is_streaming || keep_alive)
    {
        stop_stream ();
    ...
    if (!disposed && stub && old_handle >= 0)
    {
        ...
        grpc::Status s = stub->Amplifier_Dispose (&ctx, request, &response);
        safe_logger (
            spdlog::level::info, "EDX direct dispose ok={} msg={}", s.ok (), s.error_message ());
        disposed = s.ok ();
    }
    ...
    if (!disposed && old_handle >= 0)
    {
        ...
        safe_logger (spdlog::level::warn,
            "EDX direct dispose failed, trying fresh gRPC connection with old handle {}",
            old_handle);
        stub.reset ();
        grpc_channel.reset ();
        ...
        if (conn_res == (int)BrainFlowExitCodes::STATUS_OK)
        {
            ...
            ctx.set_deadline (std::chrono::system_clock::now () +
                std::chrono::seconds (std::max (1, params.timeout)));
            grpc::Status s = stub->Amplifier_Dispose (&ctx, request, &response);
            safe_logger (spdlog::level::info, "EDX fresh-conn dispose ok={} msg={}", s.ok (),
                s.error_message ());
            disposed = s.ok ();
        }
        else
        {
            safe_logger (
                spdlog::level::warn, "EDX recovery: create_grpc_stub failed = {}", conn_res);
        }
    }
...
bool AntNeuroEdxInternalBoard::parse_bool_flag (const std::string &value, bool &flag)
{
    if (value == "0")
    {
        flag = false;
...
int AntNeuroEdxInternalBoard::validate_sampling_rate (int value)
{
    if (sampling_rates_available.empty ())
    {
        return (int)BrainFlowExitCodes::STATUS_OK;
...
int AntNeuroEdxInternalBoard::validate_reference_range (double value)
{
...
int AntNeuroEdxInternalBoard::validate_bipolar_range (double value)
{
...
int AntNeuroEdxInternalBoard::parse_edx_command (const std::string &config, std::string &response)
{
...
int AntNeuroEdxInternalBoard::config_board (std::string config, std::string &response)
{
    if (!initialized)
    {
        return (int)BrainFlowExitCodes::BOARD_NOT_READY_ERROR;
...
int AntNeuroEdxInternalBoard::start_stream (int, const char *)
{
    return (int)BrainFlowExitCodes::UNSUPPORTED_BOARD_ERROR;
}
...
int AntNeuroEdxInternalBoard::config_board (std::string, std::string &)
{
    return (int)BrainFlowExitCodes::UNSUPPORTED_BOARD_ERROR;
}
...
int AntNeuroEdxInternalBoard::configure_stream_params (void *)
{
    return (int)BrainFlowExitCodes::UNSUPPORTED_BOARD_ERROR;
}
...
int AntNeuroEdxInternalBoard::parse_edx_command (const std::string &, std::string &)
{
    return (int)BrainFlowExitCodes::UNSUPPORTED_BOARD_ERROR;
}
...
bool AntNeuroEdxInternalBoard::parse_bool_flag (const std::string &, bool &)
{
    return false;
}
