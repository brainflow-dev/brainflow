...
{
    int (*func_get_data) (BLUETOOTH_HANDLE, void *, int);
    func_get_data =
        (int (*) (BLUETOOTH_HANDLE, void *, int))dll_loader->get_address ("bluetooth_get_data");
    ...
    while (keep_alive)
    {
        int res = func_get_data (bluetooth_handle, temp_buffer, buf_size);
        if (res != buf_size)
        {
            safe_logger (spdlog::level::warn, "failed to read {} bytes, read {}", buf_size, res);
            continue;
        }
...
{
    int (*func_open) (int, char *, BLUETOOTH_HANDLE *) = (int (*) (
        int, char *, BLUETOOTH_HANDLE *))dll_loader->get_address ("bluetooth_open_device");
    if (func_open == NULL)
    {
        safe_logger (
    ...
    return func_open (1, const_cast<char *> (params.mac_address.c_str ()), &bluetooth_handle);
}

int Enophone::call_stop ()
{
    int (*func_close) (BLUETOOTH_HANDLE *) =
        (int (*) (BLUETOOTH_HANDLE *))dll_loader->get_address ("bluetooth_close_device");
    if (func_close == NULL)
    {
        safe_logger (
    ...
    return func_close (&bluetooth_handle);
}
