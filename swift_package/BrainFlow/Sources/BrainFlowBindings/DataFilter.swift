...
struct DataFilter {
    ...
    **/
    ...
    static func performICA(data: [[Double]], numComponents: Int, channels: [Int]? = nil) throws ->
        ...
        if let channelIndexes = channels {
            channelsToUse = channelIndexes
        } else {
            let indexRange = 0..<data.numRows()
            channelsToUse = Array(indexRange)
        }
        
        let numCols: Int = data[0].count
        var data1d: [Double] = Array(repeating: 0.0, count: channelsToUse.count * numCols)

        var w: [Double] = Array(repeating: 0.0, count: numComponents * numComponents)
        var k: [Double] = Array(repeating: 0.0, count: channelsToUse.count * numComponents)
        var a: [Double] = Array(repeating: 0.0, count: channelsToUse.count * numComponents)
        var s: [Double] = Array(repeating: 0.0, count: numCols * numComponents)
        
        let channelTuples = Array(channelsToUse.enumerated())
        channelTuples.forEach { (i, channel) in
            for j in 0..<numCols {
                data1d[j + numCols * i] = data[channel][j]
            }
        }

        let errorCode = perform_ica(&data1d, Int32(channelsToUse.count), Int32(numCols),
...
extension Array where Element == [Double] {
    ...
    func numRows() -> Int {
        let numCols = self.map { $0.count }
        return numCols.count
    }
    
    // Return true only if the 2D array is rectangular, i.e., all rows are the same length.
